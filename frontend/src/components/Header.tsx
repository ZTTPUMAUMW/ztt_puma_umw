...
export default function Header() {
  ...
  const headerClass = `${styles['site-header']} ${isSidebarOpen ? styles['site-header--sidebar-open'] : ''}`.trim();
  ...
  return (
    <header className={headerClass}>
      <div className={styles['site-header__container']}>
        ...
        <nav className={styles['site-header__nav']}>
          <ul className={styles['site-header__menu']}>
            {menuItems.map((item) => (
              <li key={item.href} className={`${styles['site-header__menu-item']} ${isActive(item.href) ? styles['site-header__menu-item--active'] : ''}`}>
      ...
      >
        <div className={styles['site-header__container']} style={{ boxShadow: 'none', borderBottom: 'none', background: 'none', position: 'static', zIndex: 1 }}>
          <div className={styles['site-header__logo']}>
            <Link href="/" onClick={closeSidebar}>
              <Image 
                src="/logo.svg" 
          ...
          <button className={styles['site-header__x-button']} id="x-sidebar" aria-label={tCommon('closeMenu')} onClick={closeSidebar}>
            <span aria-hidden="true"></span>
          </button>
        </div>
        <ul className={styles['site-header__sidebar__menu']}>
          {menuItems.map((item) => (
            <li key={item.href} className={`${styles['site-header__sidebar__menu-item']} ${isActive(item.href) ? styles['site-header__sidebar__menu-item--active'] : ''}`}>
