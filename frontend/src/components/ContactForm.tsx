...
export default function ContactForm({
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (timeSpent < 3000) {
      ...
      setMessage({
        type: "error",
        text: "Formularz został wysłany zbyt szybko. Proszę spróbować ponownie.",
      });
      return;
    }
    ...
    if (timeSpent > 15 * 60 * 1000) {
      setMessage({
        type: "error",
        text: "Sesja wygasła. Odśwież stronę i wypełnij formularz ponownie.",
      });
      return;
    }
      ...
      <form onSubmit={handleSubmit} className={styles["contact-form"]} noValidate>
        ...
        <div className={styles["form-group"]}>
          ...
          {files.length > 0 && (
            <div className={styles["text-muted"] + " " + styles["small"]}>
              {t("totalSize", {
                current: formatFileSize(getTotalFileSize()),
                max: maxTotalFileSize / 1024 / 1024,
              })}
            </div>
          )}
