        ...
        <nav className={styles['research-areas__nav']} aria-label="Obszary badawcze">
          {mindmapData.map((item, index) => (
            <button
              ...
              type="button"
            >
              <span className={styles['research-areas__nav-number']} aria-hidden="true">{String(index + 1).padStart(2, '0')}</span>
              <span className={styles['research-areas__nav-title']}>{t(`areas.${item.key}.title`)}</span>
            </button>
          ))}
        </nav>
        
        <div className={styles['research-areas__content']} role="region" aria-live="polite" aria-label="Szczegóły obszaru badawczego">
          {activeItem && (
            <article className={styles['research-areas__card']}>
              <div className={styles['research-areas__card-header']}>
                ...
                <h3 className={styles['research-areas__card-title']}>{t(`areas.${activeItem.key}.title`)}</h3>
              </div>
              <p 
                className={styles['research-areas__card-description']}
                dangerouslySetInnerHTML={{ __html: italicizeLatinWords(t(`areas.${activeItem.key}.description`)) }}
              />
            </article>
          )}
        </div>
